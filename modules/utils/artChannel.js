...
module.exports = class ArtChannelCommand extends global.utils.baseCommand {
    ...
    async task(ctx) {
        ...
        if (!artchannels)
            await ctx.globalDb.set("artchannels", []);
        artchannels = await ctx.globalDb.get("artchannels");
        if (ctx.args.enabled === "enable") {
            if (artchannels.map(channel => channel.id).includes(ctx.args.channel.id))
                artchannels = artchannels.filter(channel => channel.id !== ctx.args.channel.id);
            artchannels.push({
