...
module.exports = class ApproveCommand extends global.utils.baseCommand {
    ...
    async task(ctx) {
        ...
        let appTch = ctx.client.channels.get("455252710732595211");
            var message = await appTch.fetchMessage(ctx.args.id);
            var rewardAmount = await ctx.db.get(`challenges/${ctx.args.difficulty}`);
            var approved = new RichEmbed()
