...
module.exports = class DisableChallenge extends global.utils.baseCommand {
	constructor(client) {
		super(client, {
			name: "disablechallenge",
			description: "Disable a challenge with a given id.",
			group: "challenges",
			memberName: "disablechallenge",
			aliases: ["disablech"],
	...
	async task(ctx) {
		...
		challengeData.challenges[index].enabled = false;
		const challenge = challengeData.challenges[index];
		await ctx.db.set("challengeData", challengeData);
		return ctx.embed({
			fields: [
				{
					name: `Challenge #${challenge.id} successfully disabled.`,
