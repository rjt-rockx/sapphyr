...
module.exports = class AwardUserCommand extends global.utils.baseCommand {
	...
	async task(ctx) {
		let noReason = new RichEmbed().setTitle("Missing Arguments").setColor("#7959ff")
		...
		if (ctx.args.amount === 0) return null;
		let response = null;
		...
		if (typeof response.error === "undefined") {
			let Success = new RichEmbed().setTitle("Success").setColor("#7959ff")
				.setDescription(`Successfully awarded ${ctx.args.amount} ${botInfo.bot.currency.sign} to ${ctx.args.user.username}`);
			await ctx.send(Success);
			let embed = new RichEmbed().setTitle("Award").setColor("#7959ff")
