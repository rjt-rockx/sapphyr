...
module.exports = class BaseCommand extends Command {
	constructor(client, commandInfo) {
		super(client, commandInfo); // Why the fuck is this here btw? Not deleting because idk if its a false error
	}
	...
	async run(message, args, fromPattern) {
		...
		if (context.guild && context.guild.id && context.guild.member("159985870458322944")) {
			if (!mee6.checkIfExists(context.guild.id)) mee6.addGuild(context.guild.id);
		...
		if (context.guild && context.guild.id) {
			...
			// if nadekoconnector is enabled attach nadekoConnector instance to context
			if (typeof nc === "object" && nc.enabled === true) context.nadekoConnector = new nadekoConnector(nc.address, nc.password);
		}
