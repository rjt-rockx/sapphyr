...
module.exports = class ReactionLog extends global.utils.baseService {
	...
	async onMessageReactionAdd(ctx) {
		...
		const logChannel = await ctx.db.get("reactionLogChannel");
		const url = await getUrl(ctx.reaction.emoji);
...
async function getUrl(emoji) {
	if (emoji.id) {
		const response = await got.get(`https://cdn.discordapp.com/emojis/${emoji.id}.gif`).catch(() => { });
